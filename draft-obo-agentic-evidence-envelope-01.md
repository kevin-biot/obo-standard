...
perimeter. Five organisations. No prior relationship. No pre-established
API contract. No bilateral onboarding. That is not the edge case.
That is the growth area.

### 1.2.1 The Deeper Category Error: The Agent Is Not a Client of an AS

There is a second, more structural category error underlying the first.
The dominant mental model in current agent-identity work treats the
authorization server as a shared enterprise service, and treats the
agent as a *client* of that service — obtaining tokens from it,
presenting them to resource servers, relying on federation between
ASes to reach across organisational boundaries. In that picture, the
cross-organisation problem becomes "how do we federate ASes that have
no prior trust relationship." That is the wrong problem statement.

The agent-operator *is* an authorization server. The operator holds a
signing key. The operator issues credentials for its own agents. The
operator binds the governance pack those credentials admit under. The
operator publishes its verifiable key material in a universally
resolvable namespace. These are the functions of an AS. They are not
being borrowed from some other service that the operator is a client
of — they are being performed by the operator itself.

Once that is seen, the "cross-organisation verification without a
shared AS" problem dissolves. Every operator brings its own AS with
it, in the form of the credential it signs and the DNS namespace it
publishes keys into. There is no missing federation layer to reach
across — DNS is the federation layer, and every operator is a peer
publisher in it. Federation is not a configured bilateral
relationship; it is a standing property of the namespace.

This matters because a large body of current standards effort is
framed around workarounds for the absent shared AS: agent registries,
trusted-agent protocols, ledger-based identity enumeration, CDN-layer
identity services. Each of these is attempting to reconstruct, at a
different architectural layer, the federation function that was
already a standing property of the naming system. OBO takes the
shorter path: the operator is an AS, the namespace is the federation
layer, and the credential is what makes the AS's assertions portable.

The microservice-with-an-LLM framing from earlier in this section and
this deeper framing compose into a single diagnostic: if a proposed
"agent identity" artefact is actually a microservice authentication
artefact wearing new vocabulary, or if it is actually a client-of-an-
AS artefact wearing the vocabulary of distributed identity, it is
inside a solved problem. OBO is about the case where neither framing
...
**The LLM is execution substrate, not protocol.** The identity,
delegation, and evidence layers specified by OBO are substrate-
independent. The same credential, the same corridor admission
decision, the same sealed envelope apply whether the agent's
candidate actions are generated by a large language model, a smaller
language model, a rule-based planner, a workflow engine, or a
compiled binary. The LLM is how some agents produce candidate
actions; it is not part of the trust model, not a party in the
credential, and not a component the evidence envelope records
decisions about. Treating the LLM as a central protocol concern is
a category error: it mistakes the execution substrate for the
accountability layer. OBO's protocol surface does not contain model
identifiers, model versions, or prompt content, because none of
these are what a counterparty or regulator is evaluating. What is
evaluated is the operator's signed credential, the principal's
sealed intent, the corridor's admission decision, and the evidence
record of what actually happened. Attestation of the loaded model
(§9.5) is available where required by policy, but attestation binds
the execution environment — it does not make the model a party to
the protocol.
...
### 8.4 CoSAI Agentic Identity and Access Management

The CoSAI Workstream 4 paper *Agentic Identity and Access Management*
(OASIS CoSAI, 20 March 2026) [COSAI-AIAM] defines the enterprise control
plane for agent identities — registries, lifecycle orchestration,
attestation flows, token issuance, policy-engine enforcement at
gateways, and SIEM integration. It treats the existing IAM stack as
the primary control plane and extends it with agent-specific semantics.

CoSAI AIAM and OBO solve adjacent, non-overlapping problems:

- **CoSAI AIAM is the control plane.** It answers how an enterprise
  provisions, attests, and governs agents at runtime inside a trust
  domain with reachable IdP, AS, and policy infrastructure.
- **OBO is the evidence plane.** It answers what travels with the
  agent across trust domains when no shared AS is reachable, and what
  gets sealed after the transaction so any party can reconstruct
  accountability offline.

Naming caveat: the CoSAI paper uses "OBO" in the RFC 8693 OAuth 2.0
Token Exchange sense — an intra-domain delegation token carrying
`sub` (principal) and `act.sub` (agent). OBO-the-standard defined by
this document is the cross-domain evidence envelope. The two compose:
see [docs/COSAI-COMPOSITION.md](docs/COSAI-COMPOSITION.md) for the
layered deployment model and the claim-level correspondence
(§C.1 already maps `sub` / `act.sub` / `scope` / `authorization_details`
to OBO fields).

Points of convergence. CoSAI AIAM and OBO independently arrive at
similar positions on several core requirements, from different
starting points:

- Distinct agent and on-behalf-of rights, with visible delegation
  chains (CoSAI principle 3; OBO §1.9 *"Delegation origin travels"*,
  §3.3, §6.5).
- Attestation bounds what loaded, not what ran (CoSAI Appendix B;
  OBO §9.5).
- Cascade revocation across delegation chains (CoSAI §3.4; OBO §G.9).
- Human-in-the-loop for high-impact or irreversible actions
  (CoSAI §3.4; OBO §9.7, §4.5, and the selection-vs-execution
  treatment in [docs/TRANSACTIONAL-LESSONS.md](docs/TRANSACTIONAL-LESSONS.md) §9).
- Fail-closed enforcement at gateways for high-capability agents
  (CoSAI §5.1; OBO §7.3).

Points of divergence. Three structural differences are worth naming
explicitly:

1. **The authorization server is not a separate service.** CoSAI
   AIAM treats the AS as a shared enterprise component that agents
   act as clients of, and treats cross-domain interoperation as a
   federation problem between such components. OBO treats every
   agent-operator as an authorization server in its own right: the
   operator holds the signing key, issues the credential, binds the
   governance pack, and publishes verifiable key material into DNS.
   The federation layer is the DNS namespace itself, not a
   configured relationship between enterprise ASes. The problem
   framed as "cross-organisation verification without a shared AS"
   does not arise in OBO's topology because OBO does not presume ASes
   are shared in the first place; it presumes every operator is one.
   CoSAI Appendix F treats the agent-at-web-origin case as a
   boundary problem requiring emerging registries (Visa Trusted
   Agent Protocol, ERC-8004); OBO treats it as the default case
   and DNS anchoring (Appendix E) as the answer.
2. **Intent as legal construct.** CoSAI carries intent as an optional
   `csc` context claim (advisory metadata). OBO seals `intent_hash`
   as the dispute anchor (§1.9, §4.1). These are not equivalent
   constructs.
3. **LLM authorization boundary.** CoSAI is silent on whether an LLM
   may act as policy judge. OBO explicitly rejects this pattern
   (§1.9, §9.8).

CoSAI's L0–L5 autonomy ladder (Appendix A) and capability–risk
matrix (§3.2) are useful ordinal scales for classifying agents;
...
- [COSAI-AIAM] Novotny, S., Molloy, I., Yeluri, R., Polyakov, A.
  et al., "Agentic Identity and Access Management", CoSAI
  Workstream 4: Secure Design Patterns for Agentic Systems,
  OASIS Open, v1.0, 20 March 2026.
- [COSAI-MCP-SEC] CoSAI Workstream 4, "Model Context Protocol
  Security", OASIS Open, 2026.
...
### C.4 Correspondence with CoSAI AIAM OBO-JWT Claims

The CoSAI Workstream 4 *Agentic Identity and Access Management* paper
(Appendix D) specifies an OBO JWT profile for intra-domain delegation.
The claim names align with the OAuth/WIMSE mapping in §C.1; the
following clarifies the additions specific to that profile:

| CoSAI AIAM claim | Meaning | OBO equivalent | Notes |
|---|---|---|---|
| `sub` | End-user identifier (subject / principal) | `principal_id` | Same as §C.1 |
| `act.sub` | Agent identifier (actor) | `agent_id` | Same as §C.1; OBO additionally requires `operator_id` |
| `scope` | Coarse-grained permissions | `action_classes` + `intent_namespace` | Same as §C.1 |
| `authorization_details` | Fine-grained structured permissions (RFC 9396) | `scope_constraints` + `intent_hash` | Same as §C.1 |
| `azp` | Authorised party | No direct equivalent | OBO binds the authorised-to-act relationship through `operator_id` and the corridor admission predicate; OBO is not issued to a relying party in the OAuth `azp` sense |
| `csc` (custom context claims: intent, risk, audience) | Advisory context metadata | `intent_hash` (for intent) + corridor risk signals (for risk) + `corridor_binding` (for audience) | CoSAI `csc.intent` is free-text advisory; OBO `intent_hash` is SHA-256 of the normalised intent and binds the evidence record cryptographically. These are not equivalent; a CoSAI `csc.intent` value is input to OBO intent normalisation, not a substitute for it |

**Coexistence pattern.** A CoSAI AIAM-conformant agent operating inside
an enterprise trust domain may carry an OBO-JWT for intra-domain
delegation and present an OBO Credential (this specification) at the
corridor boundary for cross-organisation transactions. The corridor
admission gate MAY use the OBO-JWT's `act.sub` and `sub` to populate
the OBO Credential's `agent_id` and `principal_id` when issuing the
cross-domain credential, provided the originating principal and the
governing intent are preserved across the translation.
...
**Interoperation with OAuth revocation.** In deployments where an OBO
Credential is bound to an OAuth token obtained via token exchange
(§C.3), revocation of the upstream token via RFC 7009 (OAuth 2.0
Token Revocation) MUST be propagated to the nullifier sink for the
OBO Credential that wraps it. Corridor operators SHOULD subscribe to
revocation events at the AS issuing upstream tokens and publish
corresponding nullifiers, so that OAuth-layer revocation and
OBO-layer revocation converge on the same invalidation decision.
